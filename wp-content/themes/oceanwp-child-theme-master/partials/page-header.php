...
if(is_singular('tribe_events')) {
    $title = get_the_title($post_override->ID);
} elseif(is_archive()) {
    $tax = $wp_query->get_queried_object();
    $title = $tax->name === 'tribe_events' ? __('Eventi yoga', 'futureyoga') : $tax->name;
} else {
    $title = wp_kses_post( oceanwp_has_page_title());
}
?>
...
<header class="<?php echo esc_attr( $classes ); ?>">
	...
	<div class="container clr page-header-inner">
		...
		if ( oceanwp_has_page_header_heading() ) {
			...
			<<?php echo esc_attr( $heading ); ?> class="page-header-title clr"<?php oceanwp_schema_markup( 'headline' ); ?>><?php echo $title; ?></<?php echo esc_attr( $heading ); ?>>
