...
class Plugin {
	...
	public function init() {
		...
		if ( current_user_can( 'install_plugins' ) ) {
			$today = date( "d-m-Y" );
			if ( $today >= '21-11-2022' && $today <= '28-11-2022' ) {
				$this->promo_notice_black_friday_2022();
			} else {
				$this->promo_notice();
			}		
		}
	...
	/**
	 * Promo Notice Black Friday 2022
	 *
	 * @return void
	 */
	public function promo_notice_black_friday_2022() {
		$msg = sprintf( __( '%1$sBlack Days has arrived! Buy now Dynamic.ooo - Dynamic Content for Elementor and save 40&#37;%2$s using promo code %3$sBLACKDAYS%4$s.', 'dynamic-visibility-for-elementor' ) . '<br />', '<a target="_blank" href="https://dnmc.ooo/visibilityblackdays22">', '</a>', '<strong>', '</strong>' );
		$msg .= sprintf( __( 'We give you %1$sover 140 features for Elementor%2$s that will save you time and money on achieving complex results.', 'dynamic-visibility-for-elementor' ), '<strong>', '</strong>' );
		$this->notices->info( $msg, 'black_friday_2022' );
	}
